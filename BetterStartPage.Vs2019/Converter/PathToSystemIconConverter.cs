...
namespace BetterStartPage.Converter
{
  internal class PathToSystemIconConverter : IMultiValueConverter
  {
    ...
    private object ConvertFromCache(string fileName, bool smallIcon)
    {
      ...
      {
        // were deailing with a folder or cMake application
        if (Directory.Exists(fileName))
        {
          if (File.Exists(Path.Combine(fileName, "CMakeLists.txt")))
          {
            extension = "cmake";

            if (!cache.TryGetValue(extension, out image))
            {
              Bitmap bitmap = BetterStartPage.Properties.Resources.cmake;
